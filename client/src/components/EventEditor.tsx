...
export function EventEditor({
  ...
  const set = (patch: Partial<Draft>): void => {
    ...
    setPlan(null)
    setAcknowledged(false)
    // Forget what was last asked for, because the plan it produced has just
    // been dropped. Without this, typing 18 and then putting 20 back — or any
    // edit that lands on the value already previewed — leaves `lastSent`
    // matching the draft, so the preview effect short-circuits and never
    // refetches. The screen then sits with no preview and nothing pushable
    // until the person changes something else, which looks like the form has
    // stopped working.
    lastSent.current = null
  }
