...
export class SqliteLiveryStore implements LiveryRecorder {
  ...
  static async open(path: string, options: OpenOptions = {}): Promise<SqliteLiveryStore> {
    ...
    migrate(db, "liveries", MIGRATIONS)
  ...
  /**
   * One livery's files, and nothing else's.
   *
   * This is what makes a streaming carset build possible: the archive is
   * written a driver at a time, so the heap holds one skin rather than the
   * whole pack. `read` below loads everything and is now only worth using when
   * everything is genuinely wanted.
   */
  async filesFor(
    championshipId: string,
    carModel: string,
    ...
    const files = this.#db
      .prepare(
        `SELECT f.livery_id, f.name, f.body FROM livery_file f
         ...
         WHERE l.championship_id = ? AND l.car_model = ? AND l.driver_name = ?
         ORDER BY f.name`,
      )
      .all(championshipId, carModel, driverName) as unknown as FileRow[]
    return files.map((file) => ({ name: file.name, bytes: new Uint8Array(file.body) }))
  }
  ...
  async carsetLink(championshipId: string, at: Date = new Date()): Promise<string> {
    // One transaction, and an upsert that returns the winner's slug rather than
    // throwing. Two drivers running /livery carset in the same second both
    // found no row and both inserted; the loser got a primary-key violation out
    // of an operation that is supposed to be idempotent.
    this.#db.exec("BEGIN IMMEDIATE")
    try {
      const existing = this.#db
