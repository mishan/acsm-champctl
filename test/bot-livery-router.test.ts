...
import { LiveryRouter, liveryRouterSettings } from "../src/bot/livery-router.js"
...
describe("liveryRouterSettings", () => {
  // The router's own tests build it directly, so nothing saw that serve() was
  // dropping championshipId on the floor — and the refusal it exists to answer
  // tells the admin, by name, to set exactly that key. A league running a
  // second series had no working /livery commands and no way to get any.
  it("carries the pinned championship through from the profile", () => {
    expect(liveryRouterSettings({ championshipId: CHAMP })).toMatchObject({
      championshipId: CHAMP,
    })
  })

  it("carries the clamp, the dial and the upload URL", () => {
    expect(
      liveryRouterSettings({
        channelIds: ["111111111111111111"],
        roleIds: ["222222222222222222"],
        autoApply: true,
        uploadBaseUrl: "https://liveries.example",
      }),
    ).toEqual({
      clamp: { channelIds: ["111111111111111111"], roleIds: ["222222222222222222"] },
      autoApply: true,
      uploadBaseUrl: "https://liveries.example",
    })
  })

  it("leaves everything absent when the block is empty", () => {
    // An empty clamp is unrestricted, and absent autoApply is off. Neither may
    // turn into `undefined` keys that read as configured-and-empty.
    expect(liveryRouterSettings({})).toEqual({ clamp: {} })
  })
})
