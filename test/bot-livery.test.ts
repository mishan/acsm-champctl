...
/**
 * `body` is a convenience here and `download` is what `handleUpload` takes. The
 * distinction is the point of the clamp test below: the bytes are fetched by
 * calling the thunk, so a test can watch whether that happened.
 */
const upload = (over: Partial<Parameters<typeof handleUpload>[0]> & { body?: Uint8Array } = {}) => {
...
describe("handleUpload", () => {
  ...
  it("checks the clamp before fetching anything", async () => {
    // A driver in the wrong channel should not have their file fetched to find
    // out. This used to be true of unpacking and false of downloading: the
    // caller's options object evaluated `body: await attachment.download()`
    // before handleUpload could refuse, so somebody with no role could make the
    // bot pull whatever a boosted guild allows into memory, repeatedly.
    let fetched = false
  ...
  it("checks identity before fetching anything either", async () => {
    // Same argument one step along: an unclaimed driver's file is not worth
    // bandwidth, and this is the refusal every new driver hits first.
    let fetched = false
    const result = await upload({
      claim: undefined,
      download: async () => {
  ...
  it("refuses a multi-class championship while the driver is still looking", async () => {
    // planLiveries refuses a second class, and nothing in the bot ran it — so
    // the driver was told "queued for an admin", and the refusal landed at
    // drain time in a channel nobody reads. It reads names and models and never
    // files, so it can be asked before the zip is even fetched.
    let fetched = false
    const result = await upload({
      championship: championship({
        Name: "September 2026",
        Classes: [
          championshipClass({ ID: "c1", Entrants: entryList([person({ Name: "Misha" })]) }),
          championshipClass({ ID: "c2", Entrants: entryList([person({ Name: "postaL" })]) }),
        ],
      }),
      download: async () => {
        fetched = true
    ...
    expect(result).toMatchObject({ ok: false, reason: "plan" })
    expect(result.reply).toMatch(/2 classes/)
    expect(fetched).toBe(false)
  })

  it("refuses a name the entry list holds twice", async () => {
    // resolveUploader takes the first match and says nothing. planLiveries
    // refuses the whole pack, so one doubled name in ACSM used to block every
    // other driver's queued livery at the next drain.
    const result = await upload({ championship: champ(["Misha", "Misha"]) })
    expect(result).toMatchObject({ ok: false, reason: "plan" })
    expect(result.reply).toMatch(/appears 2 times/)
  })

  it("queues nothing when the plan refuses", async () => {
    const queue = fakeQueue()
    await upload({ queue, championship: champ(["Misha", "Misha"]) })
...
describe("what the driver is told", () => {
  ...
  it("names a round the livery will never reach", () => {
    // unreachableRounds existed and only the CLI ever looked at it. The
    // assignment lands in the database and never reaches the track, so the
    // driver joins that race, sees the old car and uploads again — which is
    // the retry loop the rest of this reply is written to stop.
    const reply = uploadReply({ ...facts, unreachableRounds: [3] })
    expect(reply).toMatch(/round 3 has its own entry list/)
    expect(reply).toMatch(/won't show there/)
  })

  it("pluralises more than one unreachable round", () => {
    expect(uploadReply({ ...facts, unreachableRounds: [2, 5] })).toMatch(/rounds 2, 5 have/)
  })

  it("says nothing about rounds when the livery reaches all of them", () => {
    expect(uploadReply(facts)).not.toMatch(/own entry list/)
  })
...
describe("handleClaim", () => {
  ...
  it("claims then uploads, which is the whole flow", async () => {
    ...
    const result = await upload({ claim: await store.forDiscordUser(CHAMP, MISHA), queue })
